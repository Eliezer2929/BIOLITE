// asistencia.js
// Vista "Reporte de Marcaciones" con datos simulados desde una API de ejemplo y paginación en el lado del cliente.
// Expone el método público: window.Asistencia.render(container, { limit? }) para renderizar la vista.

/**
 * Módulo autoejecutable que gestiona la vista de asistencia de empleados.
 * - Obtiene datos ficticios desde la API randomuser.me.
 * - Simula parámetros de asistencia (presente, tardanzas, horas extras, etc.).
 * - Renderiza métricas, tabla de registros y controles de paginación.
 */
(function () {

  // URL de la API de ejemplo (randomuser.me)
  const API = "https://randomuser.me/api/?results=40&nat=es,us,br,mx,co,pe,cl,ar";

  // ---- Estado interno del módulo ----
  const state = {
    container: null, // Contenedor HTML donde se renderiza la vista
    rows: [],        // Filas de datos obtenidas/procesadas
    page: 1,         // Página actual en la paginación
  ...
  /**
   * Simula datos de asistencia para un empleado.
   * @returns {Object} Datos simulados (presente, tardanza, horas extra, etc.)
   */
  function simulateAttendance() {
    const present = Math.random() < 0.9; // 90% de probabilidad de estar presente
  ...
  /**
   * Calcula los indicadores (KPI) a partir de las filas de asistencia.
   * @param {Array} rows - Datos de asistencia
  ...
  /**
   * Obtiene y procesa las filas desde la API.
   * También consulta a window.Turnos para obtener el turno asignado a cada empleado.
   */
  async function fetchRows() {
    ...
    return users.map((u, idx) => {
      ...
      // Consulta el turno del empleado si la función está disponible
  ...
  /**
   * Renderiza la estructura general de la vista (título, KPIs, filtros, tabla y paginación)
   */
  ...
  /**
   * Genera el HTML de los KPIs a partir de los datos.
   */
  ...
  /**
   * Genera el HTML de la tabla de registros.
   */
  ...
  // Obtiene el subconjunto de filas correspondiente a la página actual
  ...
  function attachPaginationHandlers() {
    ...
    const go = (p) => {
      ...
      draw(); // Vuelve a renderizar la vista
    };
  ...
  /**
   * Renderiza la vista completa usando los datos en memoria.
   */
  function draw() {
  ...
  /**
   * Método público para renderizar la vista.
   * @param {HTMLElement} container - Contenedor HTML donde se renderiza la vista
   * @param {Object} opts - Opciones (ej: { limit: 20 })
   */
  ...
  // Expone la API pública en el objeto global
